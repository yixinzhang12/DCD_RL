...
# class GaussianActor(nn.Module):
#     def __init__(self, obs_dim, act_dim, hidden_size=200):
#         super().__init__()
#         self.obs_dim = obs_dim
#         self.act_dim = act_dim
#         self.mu_net = RecurrentNetwork(obs_dim, act_dim, hidden_size=hidden_size)
#         log_std = -0.5 * np.ones(act_dim, dtype=np.float32)
#         self.log_std = torch.nn.Parameter(torch.as_tensor(log_std))

#     def forward(self, obs, hidden, taken_act=None):
#         # obs: (BS, 1, obs_dim)
...
#     # def step(self, obs, hidden_pi, hidden_v):
#     #     with torch.no_grad():
#     #         pi, _, hidden_pi = self.pi(obs.unsqueeze(0), hidden_pi)  # obs: (obs_dim) -> (1,1,obs_dim)
#     #         a = pi.sample()
#     #         logp_a = pi.log_prob(a).sum(axis=-1)
#     #         v, hidden_v = self.v(obs.unsqueeze(0), hidden_v)
#     #     return a.cpu().numpy(), v.cpu().numpy(), logp_a.cpu().numpy(), hidden_pi, hidden_v

#     # def act(self, obs):
#     #     return self.step(obs)[0]
    
#     # def reset_hidden(self, batch_size=1):
#     #     # LSTM hidden states are (h0, c0)
#     #     # shape: (num_layers=1, batch_size, hidden_size)
#     #     h_pi = torch.zeros(1, batch_size, self.hidden_size)
#     #     c_pi = torch.zeros(1, batch_size, self.hidden_size)
#     #     h_v = torch.zeros(1, batch_size, self.hidden_size)
#     #     c_v = torch.zeros(1, batch_size, self.hidden_size)
#     #     return (h_pi, c_pi), (h_v, c_v)
    
# class RecurrentNetwork(nn.Module):
#     def __init__(self, in_dim, out_dim, hidden_size=200):
#         super().__init__()
#         self.lstm = nn.LSTM(input_size=in_dim, hidden_size=hidden_size, batch_first=True)
...
#         # inputs: (BS, 1, in_dim) if we do one step at a time
